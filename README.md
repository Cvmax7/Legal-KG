...
- `keyword_extractors.py` ：实现了8种关键词提取算法，每种算法作为一个类，继承自 *BaseKeywordExtractor* 基类。包含了`TF-IDF`、`YAKE`、`TextRank`、`RAKE`、`Embedding`、`MDKRank`、`Word2Vec`和`KeyBERT`等方法的实现。

- `main.py` ：主程序文件，负责加载语料库、初始化提取器、处理文本、提取关键词并保存结果。实现了对并集和交集的计算，以及结果的JSON格式化。

- `Config/stopwords/` ：包含中文停用词表，用于过滤常见无意义词汇。

- `legal_corpus/` ：存放按法律分支组织的法律文本，每个文本以 Markdown 格式存储。
...
6. *MDKRank*
   - 基于BERT的掩码机制评估词重要性
   - 通过掩码词后文档语义变化评估词的重要性
   - 改进了原始BERT结构，添加了BiLSTM、注意力机制和CRF层
   - 优化了训练过程，提高了法律文本的关键词识别准确率
...
4. 查看结果：
   结果将保存在 `./results/` 目录下的 JSON 文件中，文件名带有时间戳
